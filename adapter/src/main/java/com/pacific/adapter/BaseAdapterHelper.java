...
public abstract class BaseAdapterHelper<T> {
    ...
    public T setImageUrl(int viewId, String imageUrl) {
        ImageView view = retrieveView(viewId);
        Glide.with(view.getContext()).load(imageUrl).error(Glide.with(view.getContext()).load(R.drawable.default_p)).into(view);
        return (T) this;
    }
    ...
    public T setAdapter(int viewId, RecyclerAdapter adapter) {
